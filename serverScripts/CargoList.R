...
observeEvent(input$hideFiltersCL, {
  ...
  output$CargoListData <- renderRHandsontable({
    CargoList <- valuesCL[["CargoList"]]
    if (!is.null(CargoList)) {
      hot <- rhandsontable(CargoList,width = '100%', height = 400,
  ...
  output$CargoListData2 <- renderRHandsontable({
    hot2 <- req(valuesCL$hot2)
    hot2
  })
  ...
  observeEvent(input$CargoListData$changes$changes, {
    ...
    for (i in 1:nrow(edI$editedInfoCL)) {
      ...
      #Most complicated mother of all fucked up logic
